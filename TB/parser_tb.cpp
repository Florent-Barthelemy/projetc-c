...
int parser_tb()
{
	...
	DotLogicParserConfig parserConfig;
	parserConfig.objBuilderConfig.isDotSyntaxCompatible = true;

	SystemMessager messager("parser_tb");

	DotLogicParser parser(parserConfig);

	auto builtSystemModules = parser.dot2simLogic("dot_test_file.dot", "compil_0");

	map<string, Module*>::iterator it = builtSystemModules->find("NAND_MODULE");
	if(it == builtSystemModules->end())
		messager.ERROR<ios::failure>("Circuit do not exist");
